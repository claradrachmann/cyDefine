...
'''
parser = argparse.ArgumentParser(description="Simple script that takes a directory argument")
parser.add_argument("--dir", help="Directory path")
parser.add_argument("--unassigned", help="Directory path")
    
args = parser.parse_args()
unassigned=args.unassigned
unassigned = True if unassigned=="true" else False

out = '_w_unassigned' if unassigned else '_wo_unassigned'
dir=args.dir+out #'data/Levine13/CyAnno_data

##### Mandatory ######
handgatedFileinfo=dir+'/handgated.csv' 		## [Mandatory] hand-gated cells to be used for training
LiveFileinfo= dir+'/LivecellsTraining.csv'    	## [Mandatory] All Live cells of the samples used for handgating (i.e. training); these samples will also be labelled
unlabelledDataset= dir+'/Livecells.csv'    		## [Mandatory] All Live cells to be tested for annotation, i.e. for cell type identification
relevantMarkers = [line.strip() for line in open(dir+"/lineage_markers.txt") if line.strip()]	## [Mandatory] lineage markers used for hand-gated; based on column names in FCS/CSV file  
outdir = dir+'/../CyAnno_results'+out 					##  Mandatory when loadModel=False] any name of choice. If directory will hold saved session and labelled CSV file.	
...
######### Optional #######
threads = 39 			## (Int) number of available threads to use; -1 if all available threads are to be used
...
Findungated = unassigned		## Logical; if True then 'ungated' cells will be predicted. However, orignal sample ID (live cells) must be present in LiveFileinfo. 'ungated' cells are defined as all the cells that not the part of any of 'gated' cell population.
...
normalizeCell=False 		## Logical; if yes arcsine transformation with cofactor will we used to normalize both the handgated and unlabelled cell expression files.
...
cellCount = 10			## Minimum number of cells that should be present in the entire training dataset.
