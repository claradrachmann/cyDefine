...
Importantly, the reference tibble needs a `celltype` column, and both reference and query need columns named `sample` and `batch` if batch effects are to be corrected for. However, we expect most users to start their analysis from raw FCS files.
